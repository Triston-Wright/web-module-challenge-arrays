...
function addFlavor(newFlav, originalFlavors){
    originalFlavors.unshift(newFlav)
...
function getFlavorByIndex(originalFlavors, x){
    return originalFlavors[x];
}
...
function removeFlavorByName(originalFlavors, x){
    for (i = 0; i < originalFlavors.length; i++) {
        if (originalFlavors[i] == x){
...
function filterByWord(tempArray, x){
    let filteredArray = []
    for (i = 0; i < tempArray.length; i++){
        if (tempArray[i].includes(x)){
...
function getAverageWordLength(tempArray){
        let wordCount = 0;
        for (i = 0; i < tempArray.length; i++){
            for ( j = 0; j < tempArray[i].length; j++){
                if (tempArray[i][j] == (" ")){
                    ...
                    wordCount++  
                        ...
                        console.log("the letter at j is " + tempArray[i][j])
                     } 
            }
            wordCount++
        }

    return wordCount / tempArray.length;
}
